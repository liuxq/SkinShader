...
#ifndef MARMOSET_TREE_VERTEXLIT_CGINC
#define MARMOSET_TREE_VERTEXLIT_CGINC
...
fixed3 MarmoShadeTranslucentMainLight (float4 vertex, float3 normal)
{
	...
	return lightColor.rgb * (diffuse + backContrib * _TranslucencyColor) * exposureIBL.w;
}


fixed3 MarmoShadeTranslucentLights (float4 vertex, float3 normal)
{
	half4 exposureIBL = blendedExposure();
	...
	return 0.5 * (frontlight + backlight * _TranslucencyColor) * exposureIBL.w;
}
